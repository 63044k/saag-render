...
window.addEventListener('DOMContentLoaded', function() {
    ...
    img.src = 'person_AdobeStock_1239257467.png';
});
...
function createImageFromDataX(treeData, fileNameStem) {
    const gallery = document.getElementById('image-gallery');
    ...
    treeData.trees.forEach(tree => {
        ...
        // --- 1. Draw the Shadow ---
        const shadowRadius = treeRadius + 1 * scale;
...
function createImageFromDataXX(treeData, fileNameStem) {
    const gallery = document.getElementById('image-gallery');
    ...
    // 2. Draw the scale bands and labels
    ...
    // Draw labels every 5 meters
    ctx.font = `${Math.round(scale * 0.75)}px sans-serif`;
    ctx.fillStyle = 'white';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';

    for (let i = 0; i <= parkDim.width; i += 5) {
        if (i > 0 && i < parkDim.width) { // Don't draw at 0m or 30m to avoid crowding
            const label = `${i}m`;
            const pos_on_axis = (i + border) * scale;
            const pos_on_inverted_axis = canvas.height - ((i + border) * scale);

            // Top & Bottom labels
            ctx.fillText(label, pos_on_axis, border_px / 2);
            ctx.fillText(label, pos_on_axis, canvas.height - (border_px / 2));
            
            // Left & Right labels
            ctx.fillText(label, border_px / 2, pos_on_inverted_axis);
            ctx.fillText(label, canvas.width - (border_px / 2), pos_on_inverted_axis);
        }
    }
